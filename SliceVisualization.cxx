...
void SliceVisualization::Initialize(vtkSmartPointer<vtkStructuredPoints> points, vtkSmartPointer<vtkLookupTable> colorTable,
    vtkSmartPointer<vtkRenderer> renderer, Coordinates* OrientationData)
{
  // get data properties
  ...
  switch (this->_orientation)
  {
    case Sagittal:
      ...
      break;
    case Coronal:
      matrix[7] = point[1] * _spacing[1], matrix[0] = matrix[6] = matrix[9] = 1;
      break;
    case Axial:
...
SliceVisualization::~SliceVisualization()
{
  ...
  if (this->_sliceWidget) this->_sliceWidget = NULL;

  // remove thumb renderer
  if (this->_thumbRenderer) this->_thumbRenderer->RemoveAllViewProps();

  if (this->_renderer) this->_renderer->GetRenderWindow()->RemoveRenderer(this->_thumbRenderer);
}

void SliceVisualization::GenerateSlice(vtkSmartPointer<vtkStructuredPoints> points, vtkSmartPointer<vtkLookupTable> colorTable)
{
...
void SliceVisualization::UpdateSlice(Vector3ui point)
{
  ...
  {
    case Sagittal:
      if (this->_point[0] == point[0]) return;
      ...
      break;
    case Coronal:
      if (this->_point[1] == point[1]) return;
      ...
      break;
    case Axial:
      if (this->_point[2] == point[2]) return;
...
void SliceVisualization::UpdateCrosshair(Vector3ui point)
{
  ...
  {
    case Sagittal:
      if ((this->_point[1] == point[1]) && (this->_point[2] == point[2])) return;
      ...
      break;
    case Coronal:
      if ((this->_point[0] == point[0]) && (this->_point[2] == point[2])) return;
      horizontalpoints->InsertNextPoint(0, point3d[2], 0);
      ...
      break;
    case Axial:
      if ((this->_point[0] == point[0]) && (this->_point[1] == point[1])) return;
...
SliceVisualization::PickingType SliceVisualization::GetPickData(int* X, int* Y)
{
  ...
  else
  {
    ...
    if (this->_picker->GetViewProp() != NULL) pickedProp = Slice;
  }

  if (None == pickedProp) return pickedProp;
...
void SliceVisualization::GenerateThumbnail()
{
  ...
  { 0.0, 0.0, 0.0 };
  double p1[3] =
  { this->_boundsX, 0.0, 0.0 };
  double p2[3] =
  { this->_boundsX, this->_boundsY, 0.0 };
  double p3[3] =
  { 0.0, this->_boundsY, 0.0 };
...
void SliceVisualization::ZoomEvent()
{
  ...
  {
    double p0[3] =
    { lowerleft[0], lowerleft[1], 0.0 };
    double p1[3] =
    { lowerleft[0], upperright[1], 0.0 };
    double p2[3] =
    { upperright[0], upperright[1], 0.0 };
    double p3[3] =
    { upperright[0], lowerleft[1], 0.0 };
...
void SliceVisualization::SetSegmentationOpacity(unsigned int value)
{
  ...
  if (this->_segmentationHidden) return;

  if (this->_blendimages) this->_blendimages->SetOpacity(1, static_cast<float>(value / 100.0));
}
...
void SliceVisualization::ToggleSegmentationView(void)
{
  ...
  if (this->_blendimages) this->_blendimages->SetOpacity(1, opacity);
...
{
  if (this->_blendActor) return this->_blendActor.GetPointer();
